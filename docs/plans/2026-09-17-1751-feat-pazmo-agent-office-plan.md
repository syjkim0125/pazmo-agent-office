...
- G4 evaluator continuation: reuse the qualified controller and readonly VM against the current frozen candidate. A controller-owned input binds the stored human answer, challenge, evidence subject and answer digest; worker output assesses only the three understanding aspects and never supplies a human decision. Reserve a reviewer-purpose G4 lease in the existing global budget, retain unknown liveness, release confirmed closure before evaluating through CompletionLedger, and reject cancelled/stale/expired evidence. The UI explicitly launches assessment of its saved answer and shows progress/failure; no endpoint accepts an assessment. A read may display the current evidence while its assessment runs, but delivery and approval still require all leases closed. Permit at most two explicitly requested assessment attempts per exact answer; no automatic retry. Reuse approved G3 runtime topology. Scope: docs/tasks/U7-g4-runtime.md.
