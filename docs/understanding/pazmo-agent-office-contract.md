...
## Goal
Claw-Empire 기반의 독립 배포판에서 사용자가 프로젝트와 작업을 맡기면 PM·팀장·구현자·리뷰어가 요구사항 정리부터 구현·검토까지 협업하고, 사람이 결과를 이해·수용한 뒤 로컬 산출물을 인도한다. Office가 전체 실행 그래프와 공유 상태를 관리하고 역할별 에이전트는 해당 노드의 제한된 흐름을 수행한다. 실패하거나 검증하지 못한 작업은 완료로 표시하지 않는다. 실제 실행·격리·설치 시험과 모의 시험의 증거를 구분한다.
