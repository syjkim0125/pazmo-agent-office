...
- D8. 2026-09-23 사용자가 지정한 첫 실사용 마일스톤은 기존 로컬 Git 프로젝트 한 곳의 실제 요청→역할 그래프→화면 검증·사용자 승인→로컬 인도다. 구체적인 실행 경계와 종료 기준은 `docs/local-alpha-goal-prompt.md`를 따른다. M5의 추가 업무 유형·학습 고도화, M6의 tarball 검증 등 전체 제품 잔여 요구사항은 후속으로 보존한다. 마일스톤 통과를 이 Story 전체 Delivered로 표시하지 않는다.
