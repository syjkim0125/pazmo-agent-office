...
Remaining at the original checkpoint: PM/Lead role-graph connection after intake, public execution and question/answer UI, actual worker CE/Superpowers qualification, trusted user-facing G4 evaluation, learning retrieval, full restart/failure live coverage, and tarball installation. The continuation below supersedes the PM/Lead connection gap. Learning enhancements and tarball installation are outside the adopted first-use milestone. The worker receives the bounded kit node and Pazmo profile; installed host skills are not claimed as worker skill execution. Proposed optional kit recovery/approval seams are distinguished from current CLI commands in `docs/kit-office-integration-request.md`.

## Native planning and Story G1 continuation

Scope: `docs/tasks/U6-kit-planning.md`, starting at `9879e51`. New public intake packets use kit-role-v1; existing packets preserve their legacy behavior. The installed CLI supplies clarify/propose and investigate/plan node packets, dependency proposals, assignment identity and started/resumed capabilities. PM completion waits for an exact-revision operator G1; Lead uses the immutable Story view derived from that actual approval row. The full Story, including OUT and assumptions, is displayed before approval. Scope approval is not later contract/command approval or final G4.

Model and human responses in these tests are **fixtures**. The native CLI, SQLite ledgers, HTTP G1 handler and production service route are real. No authenticated model was called and no actual user approval was created in this continuation.

- Full regression: **314/314**, exit 0, Node 24.19.0, `/private/tmp/office-kit-planning-tests.log`. After the final bounded dependency-output/prompt refinement and exact HTTP/prompt assertions, related coordinator/operator checks: **38/38**, exit 0, `/private/tmp/office-kit-planning-focused.log`. Final TypeScript: exit 0.
- Native PM/Lead questions retain the same run and attempt, use the returned resume token and preserve the approved Story. Forged/stale G1, changed immutable source, cancellation, unknown restart, and capacity deferral are checked. Unknown execution is not replayed; a dispatch preceding capacity deferral resumes its saved token only when no model lease exists.
- RED/GREEN: confirmed-closed malformed model output previously leaked into unknown supervision. The coordinator now records the protocol failure and releases the still-owned closed slot. Lead question handling initially discarded its role/Story; addressed resume retains both. These are failure-injection/protocol tests, not model reasoning evidence.
- Actual service evidence routing was missing despite direct handler tests passing: a real CLI-launched server returned 423 for the new evidence route. The allowlist now reaches operator authorization (401 without a key; authenticated missing task returns NOT_FOUND/404). This closes the prior result UI's production-route gap.
- Chrome desktop 1280 and mobile 390 checks display Story/G1 and submit exact fixture text into waiting_lead, with no horizontal overflow or script error. The first browser assertion matched both list and detail state labels; scoping the assertion to detail passed. API responses were mocked; screenshots in `/private/tmp/office-g1-*.png` and test script `/private/tmp/office-g1-browser.mjs` are local diagnostic artifacts.
- Sequential main-context review checked approval authority, legacy compatibility, cancellation, exact source/candidate identity and the kit/SQLite write boundary. It is not an independent agent review. Simplification removed duplicate full planning packet data from native dependency outputs; only role proposal data remains.
- Story/Task checkers and diff whitespace check: exit 0. The parent Story remains Approved, so G4 was not checked. Vendor lint: exit 0 with 40 existing warnings. Compound used the lightweight non-interactive path: the updated existing learning passed mechanical claims/frontmatter checks; no semantic validator or subagents were used. No CONCEPTS.md exists, so vocabulary creation was deferred; active instructions already point to docs/solutions. Documentation complete.
- Native evidence writes precede SQLite acceptance and lease release, which are one DB transaction. These stores are not atomically committed together. A partial write stops for inspection; it never silently restarts or claims project completion. Compound updates the existing historical-round/cleanup learning rather than creating a duplicate.

Still needed for the local alpha: supported qualified runtime setup and model-start control, plan publication/execution approval controls, trusted G4 evaluation, and one actual model/user flow through the supported screen to delivery. The native planning implementation alone does not meet the milestone.
